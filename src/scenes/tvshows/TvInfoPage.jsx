...
function TvInfoPage() {
  const { id } = useParams();
  const { data: singleData ,loading } = useFetchApiForSingleData(
    singleDataUrl("tv", id) 
  );
  const imgFullSizeUrl =
  ...
  const saveFavoriteData = (image, id) => {
    const myFavoriteData =
      JSON.parse(localStorage.getItem("myFavoriteData")) || [];
    const isPresent = myFavoriteData.some((item) => item.id === id);
    if (!isPresent) {
      const newFabData = [
        ...myFavoriteData,
        { id: id, poster_path: image, type: "movie" },
      ];
  ...
  useEffect(() => {
    ...
    const watchListData =
      JSON.parse(localStorage.getItem("watchListData")) || [];
    const isPresentWatchList = watchListData.some(
  ...
  return (
    <Box padding={"40px 20px"}>
      <Box padding={"20px"} sx={loading?{display:"flex",justifyContent:'center',alignItems:'center'}:{}}>
        {loading ? (
              ...
              >
                <Box>
                  <img
                    ...
                    width={"280px"}
                    src={imageBaseUrl + singleData.poster_path}
                    alt="image1"
                  />
                </Box>
                ...
                <Box
                  padding={"20px 40px"}
                  display={"flex"}
                  sx={{ backgroundColor: sm && color.secondary[400] }}
                  justifyContent={"flex-start"}
                  flexDirection={"column"}
                >
                  <Box>
                    <Typography fontWeight={"700"} color="white" variant="h2">
                      {singleData.original_name}{" "}
                      <span>
                        ({new Date(singleData.first_air_date).getFullYear()})
                      </span>
                    </Typography>
                  </Box>
                  ...
                  <Box display={"flex"} sx={{ "& >*": { margin: "5px 20px" } }}>
                    <Box title={"My Favorite"}>
                      <IconButton
                        ...
                        color={addFav ? "success" : ""}
                        sx={{ padding: "12px" }}
                        style={{ backgroundColor: color.secondary[900] }}
                      >
                        <Favorite />
                      </IconButton>
                    </Box>
                  ...
                  <Box sx={{ "&>*": { margin: "2px 0px" } }}>
                    <Typography variant="h4" color="#cccc" fontSize={"16px"}>
                      {singleData.tagline}
                    </Typography>
                  </Box>
